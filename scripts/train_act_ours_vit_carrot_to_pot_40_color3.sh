#!/usr/bin/env bash
# Train official ACT with frozen TCC ours_vit patch tokens on the curated 40 demos.
...
run_name="act_ours_vit_carrot_to_pot_40_color3_frozen_train40_${steps}steps"
...
# Reuse the exact curated episode order from the official RN50 comparison.
...
/usr/bin/time -f "WALL=%e RSS_KB=%M" .venv/bin/lerobot-train \
  ...
  --policy.repo_id=Chipaipai/act-ours-vit-carrot-to-pot-40-color3-frozen-8k \
