#!/usr/bin/env bash
# Train ACT RN50 with color augmentation while freezing stem/layer1/layer2.
...
run_name="act_official_rn50_carrot_to_pot_40_color_late_train40_${steps}steps"
...
# Match the successful color-only run exactly. Freezing RN50 stem/layer1/layer2
...
/usr/bin/time -f "WALL=%e RSS_KB=%M" .venv/bin/lerobot-train \
  ...
  --policy.type=act_rn50_late \
  --policy.repo_id=Chipaipai/act-official-rn50-carrot-to-pot-40-color-late-8k \
