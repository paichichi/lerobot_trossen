#!/usr/bin/env bash
# Native 480x640 ours RN50 inside the complete official LeRobot ACT architecture.
...
output_dir="${ACT_OUTPUT_DIR:-$repo_root/outputs/train/act_native_rn50_carrot_100_50k}"
...
/usr/bin/time -f "WALL=%e RSS_KB=%M" .venv/bin/lerobot-train \
  ...
  --policy.type=native_rn50_act \
  --policy.repo_id=Chipaipai/act-native-rn50-carrot-100 \
  ...
  --job_name=act_native_rn50_carrot_100_50k \
  --wandb.enable=false \
  2>&1 | tee "$train_log"
