#!/usr/bin/env bash
# Train a new official ACT while updating ViT block 11 plus the final LayerNorm.
...
run_name="act_ours_vit_carrot_to_pot_40_color3_later1_lr1e6_train40_${steps}steps"
...
/usr/bin/time -f "WALL=%e RSS_KB=%M" .venv/bin/lerobot-train \
  ...
  --policy.repo_id=Chipaipai/act-ours-vit-carrot-to-pot-40-color3-later1-lr1e6-8k \
...
printf 'Later-only ViT training complete. Final checkpoint: %s\n' \
