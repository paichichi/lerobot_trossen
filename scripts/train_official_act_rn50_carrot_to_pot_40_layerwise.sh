#!/usr/bin/env bash
# Train official ACT RN50 with color augmentation and three LR parameter groups.
...
run_name="act_official_rn50_carrot_to_pot_40_color_layerwise_train40_${steps}steps"
...
# Reuse the exact episode order from the original RN50 run so only the
# fine-tuning strategy changes relative to the color-augmentation experiment.
...
/usr/bin/time -f "WALL=%e RSS_KB=%M" .venv/bin/lerobot-train \
  ...
  --policy.type=act_rn50_layerwise \
  --policy.repo_id=Chipaipai/act-official-rn50-carrot-to-pot-40-color-layerwise-8k \
  ...
  --policy.optimizer_lr_backbone_early=3e-7 \
  --policy.optimizer_weight_decay=1e-4 \
  --policy.scheduler_warmup_steps=500 \
  --policy.scheduler_decay_steps="$steps" \
  --policy.scheduler_decay_ratio=0.1 \
