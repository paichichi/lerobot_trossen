#!/usr/bin/env bash
# Train official LeRobot ACT RN50 with color-only augmentation and a 1e-6 backbone LR.
...
run_name="act_official_rn50_carrot_to_pot_40_color_lr1e6_train40_${steps}steps"
...
# Reuse the exact original 40-episode order. Color augmentation and backbone
# LR are the only optimization changes relative to the official RN50 baseline.
...
/usr/bin/time -f "WALL=%e RSS_KB=%M" .venv/bin/lerobot-train \
  ...
  --policy.type=act \
  --policy.repo_id=Chipaipai/act-official-rn50-carrot-to-pot-40-color-lr1e6-8k \
