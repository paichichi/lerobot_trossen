#!/usr/bin/env bash
# Train a new official ACT while updating only ViT LayerNorm affine tensors.
...
run_name="act_ours_vit_carrot_to_pot_40_color3_layernorm_only_lr1e6_train40_${steps}steps"
output_dir="${ACT_OUTPUT_DIR:-$repo_root/outputs/train/$run_name}"
train_log="${ACT_TRAIN_LOG:-$output_dir.train.log}"

if [[ "$allow_nonstandard" != true && ( "$steps" != 8000 || "$save_freq" != 1000 ) ]]; then
  echo "This controlled run requires ACT_STEPS=8000 and ACT_SAVE_FREQ=1000." >&2
  exit 2
fi
...
# Reuse all 40 demonstrations in the exact order and use the exact photometric
# augmentation contract of the frozen ViT baseline.
...
/usr/bin/time -f "WALL=%e RSS_KB=%M" .venv/bin/lerobot-train \
  ...
  --policy.repo_id=Chipaipai/act-ours-vit-carrot-to-pot-40-color3-layernorm-only-lr1e6-8k \
  ...
  --policy.vit_train_layer_norm_only=true \
...
if [[ "$steps" == 8000 && "$save_freq" == 1000 ]]; then
  for step in 1000 2000 3000 4000 5000 6000 7000 8000; do
...
printf 'LayerNorm-only ViT training complete. Final checkpoint: %s\n' \
