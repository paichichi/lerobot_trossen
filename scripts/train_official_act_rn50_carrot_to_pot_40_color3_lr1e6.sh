#!/usr/bin/env bash
# Train official ACT RN50 with exactly three color transforms per image.
...
run_name="act_official_rn50_carrot_to_pot_40_color3_lr1e6_train40_${steps}steps"
...
# Match the successful color-only run exactly. Selecting three rather than two
# transforms per image is the sole optimization/data-pipeline variable.
...
/usr/bin/time -f "WALL=%e RSS_KB=%M" .venv/bin/lerobot-train \
  ...
  --policy.type=act \
  --policy.repo_id=Chipaipai/act-official-rn50-carrot-to-pot-40-color3-lr1e6-8k \
