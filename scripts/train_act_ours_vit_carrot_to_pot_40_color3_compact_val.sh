#!/usr/bin/env bash
# Train a compact ACT head on frozen TCC ViT features with a fixed 32/8 split.
...
steps="${ACT_STEPS:-4000}"
batch_size="${ACT_BATCH_SIZE:-8}"
save_freq="${ACT_SAVE_FREQ:-500}"
eval_steps="${ACT_EVAL_STEPS:-500}"
...
run_name="act_ours_vit_carrot_to_pot_40_color3_compact_val20_${steps}steps"
...
if [[ "$allow_nonstandard" != true && \
      ( "$steps" != 4000 || "$save_freq" != 500 || \
        "$eval_steps" != 500 || "$eval_split" != 0.2 ) ]]; then
  echo "This comparison requires 4000 steps, save/eval every 500, and eval_split=0.2." >&2
  exit 2
fi
...
# LeRobot assigns the final eight episodes in this seeded order to validation.
...
/usr/bin/time -f "WALL=%e RSS_KB=%M" .venv/bin/lerobot-train \
  ...
  --policy.repo_id=Chipaipai/act-ours-vit-carrot-to-pot-40-color3-compact-val20-4k \
...
printf 'Compact frozen-ViT training complete. Best checkpoint: %s\n' \
  "$(<"$output_dir/best_checkpoint.txt")"
